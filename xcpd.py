...
def chooseBlocks(message, array, choosedMessage, failMessage):
    print(splitter)
    index = get_int_input(message % (choose(array)))
...
def chooseObject(player, object, message, choosedMessage, failMessage):
    print("您当前的状态：")
    print(player)
    answer = get_int_input(message % (str(object), choose(selects)))
    if answer == 1:
...
def chooseFight(player, enemy, message, winMessage, failMessage):
    print("\n\n您当前的状态：")
    print(player)
    answer = get_int_input(message % (str(enemy), "您的胜率：『%s』" % (winRate(player, enemy)), choose(selects)))
    if answer == 1:
...
class Person:
    ...
    def __str__(self):
        ...
        self.name, self.blood, self.level, self.vehicle, self.weapon, self.helmet, self.medkit)
        ...
        while self.blood > 0 and enemy.blood > 0:
            if self.blood < warningBlood and self.medkit != noneMedkit:
                answer = get_int_input(
                    "您当前血量『%d』有点危险，有一个医疗包：%s，是否使用：%s" % (self.blood, str(self.medkit), choose(selects)))
                if answer == 1:
...
vehicles = [Vehicle('哈雷摩托', 70, 10), Vehicle('悍马', 50, 5), Vehicle('悍马', 50, 5), Vehicle('吉普', 30, 10),
            Vehicle('吉普', 30, 10), Vehicle('吉普', 30, 10), Vehicle('自行车', 10, 100), Vehicle('自行车', 10, 100),
            Vehicle('自行车', 10, 100), Vehicle('自行车', 10, 100)]
weapons = [Weapon('AWM', 130, 5), Weapon('AKM', 25, 40), Weapon('AKM', 25, 40), Weapon('M416', 22, 40),
           Weapon('M416', 22, 40), Weapon('M4', 20, 35), Weapon('M4', 20, 35), Weapon('M4', 20, 35),
           Weapon('手枪', 15, 8), Weapon('手枪', 15, 8), Weapon('手枪', 15, 8), Weapon('手枪', 15, 8), Weapon('撬棍', 2, 100),
           Weapon('撬棍', 2, 100), Weapon('撬棍', 2, 100), Weapon('撬棍', 2, 100), Weapon('撬棍', 2, 100)]
helmets = [Helmet('三级头', 3, 230), Helmet('破损三级头', 3, 180), Helmet('残破三级头', 3, 140), Helmet('二级头', 2, 150),
           Helmet('二级头', 2, 150), Helmet('破损二级头', 2, 120), Helmet('破损二级头', 2, 120), Helmet('残破二级头', 2, 90),
           Helmet('残破二级头', 2, 90), Helmet('一级头', 1, 80), Helmet('一级头', 1, 80), Helmet('一级头', 1, 80),
           Helmet('破损一级头', 1, 60), Helmet('破损一级头', 1, 60), Helmet('破损一级头', 1, 60), Helmet('残破一级头', 1, 40),
           Helmet('残破一级头', 1, 40), Helmet('残破一级头', 1, 40)]
medkits = [Medkit('医疗盒', 50), Medkit('医疗包', 30), Medkit('医疗包', 30), Medkit('可乐', 10), Medkit('可乐', 10),
           Medkit('可乐', 10)]
enemies = [
    Person('大魔王', randint(50, maxBlood), randint(2, 6), randomItem(vehicles), randomItem(weapons), randomItem(helmets)),
    Person('小魔王', randint(20, maxBlood), randint(1, 5), randomItem(vehicles), randomItem(weapons), randomItem(helmets)),
    Person('霸哥', randint(10, maxBlood), randint(0, 4), randomItem(vehicles), randomItem(weapons), randomItem(helmets)),
    Person('萌妹', randint(10, maxBlood), randint(0, 3), randomItem(vehicles), randomItem(weapons), randomItem(helmets)),
    Person('马可波', randint(10, maxBlood), randint(0, 3), randomItem(vehicles), randomItem(weapons), randomItem(helmets)),
    Person('瓦特', randint(10, maxBlood), randint(0, 3), randomItem(vehicles), randomItem(weapons), randomItem(helmets))]
